...
class PPYOLO_R18VD_Exp(COCOBaseExp):
    def __init__(self):
        super().__init__()
        ...
        # --------------  training config --------------------- #
        self.max_epoch = 270
        self.aug_epochs = 270  # 前几轮进行mixup、cutmix、mosaic
    ...
    def get_model(self):
        from mmdet.models import Resnet18Vd, IouLoss, IouAwareLoss, YOLOv3Loss, YOLOv3Head, PPYOLO
        if getattr(self, "model", None) is None:
            backbone = Resnet18Vd(**self.backbone)
            # 冻结骨干网络
            backbone.freeze()
            backbone.fix_bn()
