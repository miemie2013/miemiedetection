...
class ScaleReg(nn.Module):
    ...
    def __init__(self):
        ...
        self.scale_reg = torch.nn.Parameter(torch.randn([1, ], dtype=torch.float32))
        constant_(self.scale_reg, value=1.)
...
class DGQP(nn.Module):
    ...
    def __init__(self, reg_topk=4, reg_channels=64, add_mean=True):
        ...
        self.reg_conv1 = nn.Conv2d(
                ...
                kernel_size=1)
        normal_(self.reg_conv1.weight, mean=0., std=0.01)
        constant_(self.reg_conv1.bias, value=0.)
        self.add_module(name, self.reg_conv1)
        ...
        self.reg_conv2 = nn.Conv2d(
                ...
                out_channels=1,
                kernel_size=1)
        normal_(self.reg_conv2.weight, mean=0., std=0.01)
        constant_(self.reg_conv2.bias, value=0.)
        self.add_module(name, self.reg_conv2)
...
class GFLHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.gfl_head_cls = nn.Conv2d(
                in_channels=self.feat_in_chan,
                out_channels=self.cls_out_channels,
                kernel_size=3,
                stride=1,
                padding=1)
        normal_(self.gfl_head_cls.weight, mean=0., std=0.01)
        constant_(self.gfl_head_cls.bias, value=bias_init_value)
        self.add_module(conv_cls_name, self.gfl_head_cls)

        conv_reg_name = "gfl_head_reg"
        self.gfl_head_reg = nn.Conv2d(
                in_channels=self.feat_in_chan,
                out_channels=4 * (self.reg_max + 1),
                kernel_size=3,
                stride=1,
                padding=1)
        normal_(self.gfl_head_reg.weight, mean=0., std=0.01)
        constant_(self.gfl_head_reg.bias, value=0.)
        self.add_module(conv_reg_name, self.gfl_head_reg)
        ...
        for i in range(len(self.fpn_stride)):
            ...
            scale_reg = ScaleReg()
            self.add_module(feat_name, scale_reg)
